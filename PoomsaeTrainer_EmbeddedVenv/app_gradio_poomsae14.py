...
import os, sys, tempfile, platform, math, json, zipfile, time, glob, types
import numpy as np
import cv2

# ---- Gradio/PyInstaller environment hardening ----
os.environ.setdefault("GRADIO_ANALYTICS_ENABLED", "False")
os.environ.setdefault("GRADIO_DONT_CREATE_BACKUP_PYI", "1")
...
# ✅ REMOVE the previous stub that replaced "gradio.component_meta" in sys.modules.
# It could cause version skew inside the frozen app and led to the _constructor_args error.

import gradio as gr

# --- Gradio constructor_args compatibility shim (non-recursive) ---
try:
    from gradio.components.base import Component as _GradioComponentBase
    _orig_init = _GradioComponentBase.__init__

    def _shimmed_init(self, *args, **kwargs):
        # call original init first so constructor_args is populated the normal way
        _orig_init(self, *args, **kwargs)
        try:
            # If older internals expect `._constructor_args`, ensure it's present as a plain attribute.
            if not hasattr(self, "_constructor_args"):
                val = getattr(self, "constructor_args", None)
                # store the actual dict/tuple, not a property, to avoid recursion
                if val is not None:
                    object.__setattr__(self, "_constructor_args", val)
        except Exception:
            # Never block startup if internals shift again
            pass

    _GradioComponentBase.__init__ = _shimmed_init  # monkey-patch ctor

except Exception:
    pass

# (Optional) last-resort fallback ONLY if some external code tries "from gradio.component_meta import ComponentMeta"
try:
    from gradio.component_meta import ComponentMeta  # noqa: F401
except Exception:
    class ComponentMeta(ABCMeta):  # noqa: D401
        """Fallback metaclass used only if Gradio's component_meta isn't importable."""
        pass

print("[PoomsaeTrainer] entry = app_gradio_poomsae14.py")

BASE = getattr(sys, "_MEIPASS", os.path.abspath(os.path.dirname(__file__)))
ASSETS_DIR = os.path.join(BASE, "assets")
MODELS_DIR = os.path.join(ASSETS_DIR, "models")
print("[Paths] BASE:", BASE, "ASSETS_DIR:", ASSETS_DIR, "MODELS_DIR:", MODELS_DIR)

MM_CACHE = os.path.join(tempfile.gettempdir(), "mmengine_cache")
os.makedirs(MM_CACHE, exist_ok=True)
os.environ.setdefault("MMENGINE_HOME", MM_CACHE)

if getattr(sys, "frozen", False):
    os.environ["POOMSAE_SKIP_IMPORTS"] = "1"
...
if os.getenv("POOMSAE_SKIP_IMPORTS", "0") != "1":
    _check_imports()

# Patch gradio-client schema walker for boolean schemas (older versions)
try:
...
# ------------------ Movement templates (polynomials) ------------------
...
def _prepare_mm_shims():
    import types
    # Keep MMCV from requiring CUDA ops
    ...
    try:
        ...
        mmcv_pkg = _types.ModuleType('mmcv')
        ...
        class _MSDA:
            ...
            def __call__(self, *args, **kwargs): return None
        def active_rotated_filter(*args, **kwargs): return None
        ops_mod = _types.ModuleType('mmcv.ops')
        ...
        msda_mod = _types.ModuleType('mmcv.ops.multi_scale_deform_attn')
        msda_mod.MultiScaleDeformableAttention = _MSDA
        sys.modules['mmcv.ops.multi_scale_deform_attn'] = msda_mod
        arf_mod = _types.ModuleType('mmcv.ops.active_rotated_filter')
...
def get_inferencer():
    ...
    # Preferred: RTMPose-S
    rtm_cfg_glob = os.path.join(
        MODELS_DIR, "configs", "body_2d_keypoint", "rtmpose", "coco",
        "rtmpose-s_*coco-256x192.py"
    )
    rtm_cfgs = sorted(glob.glob(rtm_cfg_glob))
    rtm_cfg = rtm_cfgs[0] if rtm_cfgs else None
    rtm_w = os.path.join(MODELS_DIR, "rtmpose", "rtmpose-s_coco-256x192.pth")

    # Detector (RTMDet-M)
    det_cfg_candidates = [
        os.path.join(MODELS_DIR, "detector", "rtmdet", "rtmdet_m_8xb32-300e_coco.py"),
        os.path.join(MODELS_DIR, "detector", "rtmdet", "rtmdet-ins_m_8xb32-300e_coco.py"),
    ]
    det_cfg = next((p for p in det_cfg_candidates if os.path.exists(p)), None)

    det_w_glob = os.path.join(MODELS_DIR, "detector", "rtmdet", "*.pth")
    det_ws = sorted(glob.glob(det_w_glob))
    det_w = det_ws[0] if det_ws else None

    if rtm_cfg and os.path.exists(rtm_cfg) and os.path.exists(rtm_w) and det_cfg:
        try:
            print("[Models]\n  POSE_CFG:", rtm_cfg,
                  "\n  POSE_WEIGHTS:", rtm_w,
                  "\n  DET_CFG:", det_cfg,
    ...
    # Fallback: TD-HM (optional, only if shipped)
    td_cfg = os.path.join(
        MODELS_DIR, "configs", "body_2d_keypoint", "topdown_heatmap", "coco",
        "td-hm_res50_8xb64-210e_coco-256x192.py"
    )
    td_w = os.path.join(MODELS_DIR, "td-hm_res50_8xb64-210e_coco-256x192.pth")
    det_cfg_td = det_cfg
    det_w_td = det_w

    if os.path.exists(td_cfg) and det_cfg_td:
        try:
            print("[Models/Fallback TD-HM]\n  POSE_CFG:", td_cfg,
                  "\n  POSE_WEIGHTS:", (td_w if os.path.exists(td_w) else None),
                  "\n  DET_CFG:", det_cfg_td,
                  "\n  DET_WEIGHTS:", det_w_td)
            return MMPoseInferencer(
                pose2d=td_cfg,
                pose2d_weights=(td_w if os.path.exists(td_w) else None),
                det_model=det_cfg_td,
                det_weights=det_w_td,
                device="cpu",
            )
        except Exception:
    ...
    missing = []
    if not (rtm_cfg and os.path.exists(rtm_cfg)):
        missing.append("RTMPose config assets/models/configs/body_2d_keypoint/rtmpose/coco/rtmpose-s_*coco-256x192.py")
    if not os.path.exists(rtm_w):
        missing.append("RTMPose weights assets/models/rtmpose/rtmpose-s_coco-256x192.pth")
    if not det_cfg:
        missing.append("RTMDet detector config assets/models/detector/rtmdet/rtmdet_m_8xb32-300e_coco.py or rtmdet-ins_*.py")
    if det_w is None:
        missing.append("RTMDet weights assets/models/detector/rtmdet/*.pth (needed for true offline)")
    if not os.path.exists(td_cfg):
        missing.append("Optional TD-HM cfg assets/models/configs/body_2d_keypoint/topdown_heatmap/coco/td-hm_res50_8xb64-210e_coco-256x192.py")
            ...
            if inst is not None:
                ...
                if xys.shape[0] != 17:
                    if xys.shape[0] > 17:
                        ...
                        pad = 17 - xys.shape[0]
                        xys = np.vstack([xys, np.full((pad, 2), np.nan, float)]); scs = np.hstack([scs, np.zeros((pad,), float)])
...
def _rmse_to_score(rmse_px: float, W: int, H: int, ok_thresh: float = 0.025):
    diag = math.hypot(W, H)
    rmse_frac = rmse_px / max(1e-6, diag)
    return float(np.clip(1.0 - (rmse_frac / ok_thresh), 0.0, 1.0))

def _rmse_frac_of_diag(rmse_px: float, W: int, H: int) -> float:
    diag = math.hypot(W, H)
    return float(rmse_px / max(1e-6, diag))

def _score_to_accuracy_pct(score: float) -> int:
    return int(round(100.0 * float(np.clip(score, 0.0, 1.0))))
...
def analyze_poomsae(video,
    ...
    try:
        ...
        for s in range(S):
            ...
            score = _rmse_to_score(best_rmse, W, H, ok_thresh=0.025)
            acc_pct = _score_to_accuracy_pct(score)
            rmse_pct = round(100.0 * _rmse_frac_of_diag(best_rmse, W, H), 2)
            per_segment.append({
                "segment": s+1,
                "start": float(times[a]),
                "end": float(times[b-1]),
                ...
                "rmse_pct_of_diag": rmse_pct
            })
        csv_lines = ["segment,start,end,best_movement,side,score,accuracy_pct,rmse,rmse_pct_of_diag,note"]
        for it in per_segment:
            csv_lines.append(
                ...
                f"{_safe_int(it.get('accuracy_pct', 0))},"
                f"{_safe_float(it.get('rmse'),float('nan')) if it.get('rmse') is not None else ''},"
                f"{_safe_float(it.get('rmse_pct_of_diag', float('nan'))):.2f},"
        ...
        valid_scores = [it["score"] for it in per_segment if it.get("rmse") is not None]
        overall_acc_pct = _score_to_accuracy_pct(float(np.nanmean(valid_scores))) if valid_scores else 0
        summary["overall_accuracy_pct"] = int(overall_acc_pct)
...
def analyze_poomsae_stream(v, segs, thr, stab, strd, mconf, overlays):
    try:
        ...
        for s in range(S):
            a, b = splits[s], splits[s + 1]
            yield json.dumps({"phase": "scoring", "segment_index": s + 1, "segments_total": S,
                              "progress_pct": round(100 * (s) / S, 2)}, ensure_ascii=False), None, None
            if b <= a + 1:
                per_segment.append({
