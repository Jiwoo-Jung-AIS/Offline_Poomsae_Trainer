...
# Safe CPU/thread setup + Windows event-loop policy
try:
    import torch  # noqa
    try:
...
def _prepare_mm_shims():
    ...
    try:
        ...
        # Minimal stub if mmcv import fails entirely
...
@lru_cache(maxsize=1)
def get_inferencer():
    """Prefer one-stage (detector-free) RTMO; fallback to top-down."""
    _prepare_mm_shims()
...
def analyze_poomsae_stream(v, segs, thr, stab, strd, mconf, overlays):
    try:
        ...
        for i, out in enumerate(gen):
            ...
            # Stream progress every ~1 processed frame (can change to 10 if too chatty)
...
with gr.Blocks(title="Poomsae 14 Analyzer", theme="default") as demo:
    ...
    # Use the streaming version so progress appears in the textbox
    go.click(
